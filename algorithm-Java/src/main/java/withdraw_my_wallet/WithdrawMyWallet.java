 ...
 */
public class WithdrawMyWallet {
	public int[] solution(int money) {
		int[] answer = new int[9];
		int[] sort_price = {50000, 10000, 5000, 1000, 500, 100, 50, 10, 1};

		for (int i = 0; i < sort_price.length; i++) {
			if (money / sort_price[i] != 0) {
				answer[i] = money / sort_price[i];
				money -= sort_price[i] * (money / sort_price[i]);
				if (money < 0) {
					break;
				}
			}
